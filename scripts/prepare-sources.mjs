...
import { PUBLIC, ROOT, SOURCE_METADATA_PATH, DEFAULT_SOURCE_ROOT, projectPath } from './lib/paths.mjs';
import { loadRegistry } from './lib/registry.mjs';

export function prepareSources({ sourceRoot = DEFAULT_SOURCE_ROOT, targetLib } = {}) {
	...
	const libraries = registry.libraries.filter(
		(lib) => !targetLib || lib.name === targetLib || lib.folder === targetLib
	);
	...
	metadata.libraries = registry.libraries
		.filter((lib) => prepared.has(lib.name))
		.map((lib) => prepared.get(lib.name));
	...
	function prepareLibrary(lib) {
		...
		const repoDir = path.join(sourceRoot, lib.repo);
...
if (isMain) {
	try {
		prepareSources({ targetLib: process.argv[2] });
	} catch (error) {
